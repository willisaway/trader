CREATE OR REPLACE PROCEDURE PRC_GP_ClassKline15F_SAVE
(
...
lastClassKline30F GP_CLASS_KLINE_15F%ROWTYPE;
jsq Number;--计数器，临时数据
sequenceNo NUMBER;
closeRef1 NUMBER;
incPer NUMBER;
...
BEGIN
--检查是否已经下载
SELECT COUNT(1) INTO jsq FROM GP_CLASS_KLINE_15F WHERE CLASSIFY_CODE = classifyCode AND PERIOD_CODE=periodCode AND ORIGIN_CODE='H';
IF jsq IS NULL OR jsq=0 THEN
    --是否有之前的K线
    SELECT COUNT(1) INTO jsq FROM GP_CLASS_KLINE_15F 
    ...
    --计算各技术指标
     ...
     IF jsq>0 THEN--存在之前的一根K线
        ...
        WHERE CLASSIFY_CODE = classifyCode 
         ...
         SELECT MAX(PRICE_HIGH) INTO H9 FROM GP_CLASS_KLINE_15F WHERE CLASSIFY_CODE = classifyCode AND SEQUENCE_NO>lastClassKline30F.SEQUENCE_NO-8;
         SELECT MIN(PRICE_LOW) INTO L9 FROM GP_CLASS_KLINE_15F WHERE CLASSIFY_CODE = classifyCode AND SEQUENCE_NO>lastClassKline30F.SEQUENCE_NO-8;
     ...
     --插入或更新K线数据
     SELECT COUNT(1) INTO jsq FROM GP_CLASS_KLINE_15F WHERE CLASSIFY_CODE = classifyCode AND PERIOD_CODE=periodCode AND ORIGIN_CODE='T';
     IF jsq>0 THEN
     --更新
       UPDATE GP_CLASS_KLINE_15F
     ...
     ELSE
     --新增
       INSERT INTO GP_CLASS_KLINE_15F(ROW_ID, CLASSIFY_CODE, SEQUENCE_NO,PERIOD_CODE, SNAP_TIME, 
