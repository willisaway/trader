CREATE OR REPLACE PROCEDURE PRC_GP_StockKline30F_SAVE
(
...
lastStockKline30F GP_STOCK_KLINE_30F%ROWTYPE;
jsq Number;--计数器，临时数据
sequenceNo NUMBER;
closeRef1 NUMBER;
incPer NUMBER;
...
--初始化复权系数 end
SELECT COUNT(1) INTO jsq FROM GP_STOCK_KLINE_30F WHERE STOCK_CODE = stockCode AND PERIOD_CODE=periodCode AND ORIGIN_CODE='H';
IF jsq IS NULL OR jsq=0 THEN
    --是否有之前的K线
    SELECT COUNT(1) INTO jsq FROM GP_STOCK_KLINE_30F 
    ...
    --计算各技术指标
     ...
     IF jsq>0 THEN--存在之前的一根K线
         --查询当前K线日期之前最近的一根K线
        SELECT * INTO lastStockKline30F
        FROM GP_STOCK_KLINE_30F  
        WHERE STOCK_CODE = stockCode 
         ...
         sequenceNo := lastStockKline30F.SEQUENCE_NO + 1;
         closeRef1 := lastStockKline30F.PRICE_CLOSE;
         closeEma5 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_5,5);
         closeEma10 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_10,10);
         closeEma20 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_20,20);
         closeEma30 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_30,30);
         closeEma60 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_60,60);
         closeEma150 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_150,150);
         volumnEma5 := EMA(tradeVolumn,lastStockKline30F.VOLUMN_EMA_5,5);
         volumnEma10 := EMA(tradeVolumn,lastStockKline30F.VOLUMN_EMA_10,10);
         volumnEma20 := EMA(tradeVolumn,lastStockKline30F.VOLUMN_EMA_20,20);
         volumnEma30 := EMA(tradeVolumn,lastStockKline30F.VOLUMN_EMA_30,30);
         volumnEma60 := EMA(tradeVolumn,lastStockKline30F.VOLUMN_EMA_60,60);
         volumnEma150 := EMA(tradeVolumn,lastStockKline30F.VOLUMN_EMA_150,150);
         amountEma5 := EMA(tradeAmount,lastStockKline30F.AMOUNT_EMA_5,5);
         amountEma10 := EMA(tradeAmount,lastStockKline30F.AMOUNT_EMA_10,10);
         amountEma20 := EMA(tradeAmount,lastStockKline30F.AMOUNT_EMA_20,20);
         amountEma30 := EMA(tradeAmount,lastStockKline30F.AMOUNT_EMA_30,30);
         amountEma60 := EMA(tradeAmount,lastStockKline30F.AMOUNT_EMA_60,60);
         amountEma150 := EMA(tradeAmount,lastStockKline30F.AMOUNT_EMA_150,150);
         closeEma6 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_6,6);
         closeEma13 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_13,13);
         closeEma12 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_12,12);
         closeEma26 := EMA(priceClose,lastStockKline30F.CLOSE_EMA_26,26);
         dif_S := (closeEma12 - closeEma26)*100/closeEma26;
         dif_F := (closeEma6 - closeEma13)*100/closeEma13;
         dea_S := lastStockKline30F.DEA_S*8/10+dif_S*2/10;
         dea_F := lastStockKline30F.DEA_F*3.5/5.5+dif_F*2/5.5;
         dea_S_Angle := ATAN(dea_S-lastStockKline30F.DEA_S)*180/3.1415926;
         dea_F_Angle := ATAN(dea_F-lastStockKline30F.DEA_F)*180/3.1415926;
         SELECT MAX(PRICE_HIGH) INTO H9 FROM GP_STOCK_KLINE_30F WHERE STOCK_CODE = stockCode AND SEQUENCE_NO>lastStockKline30F.SEQUENCE_NO-8;
         SELECT MIN(PRICE_LOW) INTO L9 FROM GP_STOCK_KLINE_30F WHERE STOCK_CODE = stockCode AND SEQUENCE_NO>lastStockKline30F.SEQUENCE_NO-8;
	 ...
	 RSV := 50;
	 END IF;
         kdj_K := 2*lastStockKline30F.KDJ_K/3+RSV/3;
         kdj_D := 2*lastStockKline30F.KDJ_D/3+kdj_K/3;
         kdj_J := 3*kdj_K-2*kdj_D;
         tr := 100*(priceHigh-priceLow)/lastStockKline30F.PRICE_CLOSE;
         IF 100*ABS(priceHigh-lastStockKline30F.PRICE_CLOSE)/lastStockKline30F.PRICE_CLOSE>tr THEN
           tr :=100*ABS(priceHigh-lastStockKline30F.PRICE_CLOSE)/lastStockKline30F.PRICE_CLOSE;
         END IF;
         IF 100*ABS(priceLow-lastStockKline30F.PRICE_CLOSE)/lastStockKline30F.PRICE_CLOSE>tr THEN
           tr :=100*ABS(priceLow-lastStockKline30F.PRICE_CLOSE)/lastStockKline30F.PRICE_CLOSE;
         END IF;
         atr := 13*lastStockKline30F.ATR/15+2*tr/15;
         incPer := (priceClose-closeRef1)/closeRef1;
         --计算量价强度
         pv_sum_1 := tradeAmount/amountEma150*incPer*100;
         pv_sum_5 := EMA(pv_sum_1,lastStockKline30F.PV_SUM_5,5);
         pv_sum_10 := EMA(pv_sum_1,lastStockKline30F.PV_SUM_10,10);
         pv_sum_20 := EMA(pv_sum_1,lastStockKline30F.PV_SUM_20,20);
         pv_sum_30 := EMA(pv_sum_1,lastStockKline30F.PV_SUM_30,30);
         pv_sum_60 := EMA(pv_sum_1,lastStockKline30F.PV_SUM_60,60);
     ELSE
         sequenceNo :=1;
         closeRef1 := priceOpen;
     ...
     --插入或更新K线数据
     SELECT COUNT(1) INTO jsq FROM GP_STOCK_KLINE_30F WHERE STOCK_CODE = stockCode AND PERIOD_CODE=periodCode AND ORIGIN_CODE='T';
     IF jsq>0 THEN
     --更新
       UPDATE GP_STOCK_KLINE_30F
     ...
     ELSE
     --新增
       INSERT INTO GP_STOCK_KLINE_30F(ROW_ID, STOCK_CODE, SEQUENCE_NO,PERIOD_CODE, SNAP_TIME, 
