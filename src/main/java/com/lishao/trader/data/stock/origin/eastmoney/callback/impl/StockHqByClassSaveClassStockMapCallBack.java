	...
	public void runCore(){
		...
		try {
			...
			for(String stockHq:stockHqList){
				...
				GpStockClassMap updateBean = gpStockClassMapService.queryByClassifyStockCode(classifyCode, stockCols[1]);
				if(updateBean!=null){
					updateBean.setDeletedFlag("0");updateBean.setLastUpdDate(new Date());
					gpStockClassMapService.update(updateBean);
				}else{
					addBean.setIncluDate(new Date());
					gpStockClassMapService.add(addBean);
				}
			}
		} catch (Exception e) {
